...
if (!isset($data['anio'])) {
    echo json_encode(["error" => "Falta el parámetro 'anio'."]);
    exit;
}
...
$sql = "SELECT r.id_res, r.nom_usu_res, r.matricula_veh, v.mar_veh, v.mod_veh, v.precio_veh, r.fec_res, r.fec_dev, r.tar_adi
        ...
        WHERE EXTRACT(YEAR FROM r.fec_res) = :anio And r.met_pag IS NOT NULL";
...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(170, 15, "Reporte Anual de Ventas", 0, 1, 'L', false);
        $this->Ln(10);
    }
...
if (!$reservas) {
    ...
    $pdf->Cell(0, 10, iconv("UTF-8", "ISO-8859-1", "Reporte del año: $anio"), 0, 1, 'L');
...
$pdf->Output('D', "reporte_anual_{$anio}.pdf");
?>
